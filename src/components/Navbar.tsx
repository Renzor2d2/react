import styles from "./Nabvar.module.css";
import NavbarButton from "./Navbutton.tsx";
import { NavList } from "./Navbutton.tsx";
import { Link } from "react-router-dom";
export default function Navbar() {
	return (
		<header className="w-[100%] bg-[#ff3b3c] p-2.5 flex flex-col items-center">
			<div
				className="w-[100%] flex flex-col items-center 
      sm:flex-row justify-between"
			>
				<Link
					to="/"
					className="w-[35%]
        sm:w-[10%]"
				>
					<img className="h-6" src="/tiendamia-logo.svg" alt="Logo store" />
				</Link>
				<form
					className="w-[100%] 
        sm:w-[50%] "
				>
					<input
						className="w-[100%] rounded"
						type="text"
						placeholder="Search"
						id="search"
					/>
				</form>
				<ul
					className="w-[100%] flex flex-row justify-around
        sm:w-[30%]"
				>
					<li id="facebook" className="">
						<Link className="h-6" to="https://facebook.com">
							<img
								className="h-10"
								src="data:image/png;base64,"
							/>
						</Link>
					</li>
					<li id="instagram" className="h-6">
						<Link className="h-6" to="https://instagram.com">
							<img
								className="h-10"
								src="data:image/png;base64,"
							/>
						</Link>
					</li>
					<li id="cart" className="h-6">
						<Link className={styles["header-a"]} to="/cart">
							<img
								className="h-10"
								src="data:image/png;base64,"
							/>
						</Link>
					</li>
					<li id="cart" className="h-6 w-[10%] ">
						<Link className="h-6" to="/">
							<img className="h-10" src="./public/user.png" />
						</Link>
					</li>
					<li id="cart" className="h-6 w-[10%] *:sm:hidden">
						<Link className="h-6" to="/">
							<img className="h-10" src="./public/hamburguer.png" />
						</Link>
					</li>
				</ul>
			</div>
			<nav
				id="navbar"
				className="hidden 
       sm:flex w-[100%]"
			>
				{NavList.map((each) => (
					<NavbarButton
						key={each.title}
						title={each.title}
						link={each.linkTo}
					/>
				))}

				{/* <NavbarButton title="Ofertas" link="#" />
				<NavbarButton title="Cómo comprar" link="#" />
				<NavbarButton title="Costos y tarifas" link="#" />
				<NavbarButton title="Mis pedidos" link="#" />
				<NavbarButton title="Garantía" link="#" /> */}
			</nav>
		</header>
	);
}
